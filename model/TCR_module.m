...
addrule(model,['TCR_MHC_tot = 0.5 * (' P_T ' + TCR_tot + ' K_D ' - TCR_tot * sqrt(( (' P_T ' + TCR_tot + ' K_D ')/TCR_tot)^2 - 4*' P_T '/TCR_tot)) '],'repeatedAssignment');
% C_T = 0.5*(P_T + T_T + K_D - sqrt((P_T + T_T + K_D)^2 -4*P_T*T_T));

alpha = ['(k_TCR_p /( k_TCR_off + k_TCR_p))'];
beta  = ['k_TCR_off /( k_TCR_off + phi_TCR)'];
C_T   = [beta ' * ' alpha '^N_TCR * TCR_MHC_tot'];
% addrule(model,['H_' antigen_name ' = ' beta ' * ' alpha '^N_TCR * TCR_MHC_tot'],'repeatedAssignment');
...
% 
% figure; hold on
% simbio_plot(simData,'TCR_MHC_M1p1_tot','LegendEntry','TCR_MHC_M1p1_tot');
% simbio_plot(simData,'C_T','LegendEntry','C_T');
% set(gca,'Yscale','log')
% figure; hold on
% simbio_plot(simData,'P_T','LegendEntry','P_T');
% % simbio_plot(simData,'TCR_M1p1_tot');
% plot(simData.time,ones(size(simData.time)).* model.parameters(102).value)
% simbio_plot(simData,'K_D','LegendEntry','K_D');
% set(gca,'Yscale','log')
